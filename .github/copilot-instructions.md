...
- **Key files / classes to inspect before changing behavior**:
  ...
  - `src/processors/data_formatter.py` — `DataFormatter.format_for_training()` creates Alpaca/ChatML/raw examples and controls chunking (`chunk_size`, `chunk_overlap`) and optional augmentation/shuffle.
